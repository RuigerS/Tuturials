...
public class Opgave6 {
    public static void main(String[] args) {
        ...
        boolean eos = false;
        while (!eos) {
            if (input.substring(0, 8).equals("turn on ")) {
                action = 1;
                input = input.substring(8);
            }
            if (input.substring(0, 9).equals("turn off ")) {
                action = 2;
                input = input.substring(9);
            }
            if (input.substring(0, 7).equals("toggle ")) {
                action = 3;
                input = input.substring(7);
            }
            System.out.println(action);
            startX = Integer.parseInt(input.substring(0, input.indexOf(',')));
            input = input.substring(input.indexOf(',') + 1);
            System.out.println(startX);
            startY = Integer.parseInt(input.substring(0, input.indexOf(' ')));
            input = input.substring(input.indexOf(' ') + 9);
            System.out.println(startY);
            endX = Integer.parseInt(input.substring(0, input.indexOf(',')));
            input = input.substring(input.indexOf(',') + 1);
            System.out.println(endX);
            endY = Integer.parseInt(input.substring(0, input.indexOf(' ')));
            input = input.substring(input.indexOf(' ') + 1);
